...
import 'package:seezn_tv_app/features/presentation/widget/detail_icons_wg.dart';
...
class _DetailScreenState extends State<DetailScreen> {
  ...
  void initState() {
    super.initState();
     context
            .read<OneVideoDataBloc>()
            .add(OneVideoDataCustomEvent(movieID: widget.videoData.id));
  }
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      ...
      child: BlocListener<OneVideoDataBloc, OneVideoDataState>(
        listener: (context, state) {
          if (state is OneVideoDataSuccess) {
            fetchData = state.fetData;
            // value = fetchData.count["count"]!.toDouble();
          }
          if (state is OneVideoDataFailure) {
            ScaffoldMessenger.of(context)
                .showSnackBar(SnackBar(content: Text("Error: ${state.message}",style: TextStyle(color: AppColor.colorWhiteFFFFFF),)));
          }
        },
        child: BlocBuilder<OneVideoDataBloc, OneVideoDataState>(
          builder: (context, state) {
            return Stack(
              children: [
               ...
               if(state is OneVideoDataLoading)
                 const Center(
